...
def workflow(
    ...
    goal: Annotated[str, typer.Option("--goal", help="Session goal.")] = "Aggressive automated reconnaissance and validation",
    profile: Annotated[str, typer.Option("--profile", help="Automation profile: auto, web, network, ad.")] = "auto",
    mode: Annotated[str, typer.Option("--mode", help="Execution mode: aggressive or safe.")] = "aggressive",
    max_tools: Annotated[int, typer.Option("--max-tools", help="Maximum auto tools to execute.")] = 10,
    show_raw: Annotated[bool, typer.Option("--show-raw", help="Show raw output preview for each tool.")] = True,
) -> None:
    if not target:
    ...
    with _db_session() as db:
        ...
        if project is None:
            ...
            console.print(f"Created new project: {project.display_name}", style="green")
        else:
            console.print(f"Using existing project: {project.display_name}", style="cyan")

        session_obj = start_session(db=db, project_id=project.id, goal=goal, operator_name="autoflow")
        console.print(f"Started session: {session_obj.id}", style="green")

        project_dir = _project_dir(project.display_name)
        sequence = _auto_recon_sequence(target=target, profile=profile, mode=mode)[:max_tools]

        for idx, (tool_name, tool_input) in enumerate(sequence, start=1):
            console.print(
                Panel.fit(
                    f"[bold cyan]Step {idx}/{len(sequence)}[/bold cyan]\n[bold]Tool:[/bold] {tool_name}\n"
            ...
            console.print(f"[{status_style}]Status:[/{status_style}] {result.status}")
